...
- Локальный Upscaler: пять AI-моделей, итоговый масштаб ×2/×3/×4 и проба до трёх секунд с синхронным сравнением.
...
В секции **Upscaler** включите «Включить AI-увеличение», выберите модель и итоговый масштаб ×2, ×3 или ×4. По умолчанию AI выключен, выбраны Nomos8k SPAN Weak и ×2. Настройки сохраняются при смене видео в открытой странице, но сбрасываются после её перезагрузки.
...
**Установка по требованию.** Нажмите «Скачать необходимые компоненты». Для Nomos8k скачивается официальный архив SPAN `20240831-055257` (16 553 410 байт), для Real-ESRGAN — `realesrgan-ncnn-vulkan-20220424-windows.zip` из релиза `v0.2.5.0` (45 474 481 байт). Архив содержит семейство моделей целиком: при переключении между установленными моделями повторная загрузка не нужна. Состав, версия и SHA-256 закреплены в каталоге приложения. При обычном запуске ничего не скачивается. Пакеты хранятся в `tools/ai`, не меняют PATH и не включаются в portable-поставку.
...
**Размер и качество.** AI получает область crop в исходном разрешении. Процент уменьшения временно блокируется; после выключения AI он снова действует с прежним значением. Итоговый масштаб считается от crop. Nomos8k и x4plus работают в родном ×4 с последующим уменьшением Lanczos до ×2/×3; AnimeVideo использует родную модель соответствующего масштаба. Выбор ×2 для модели ×4 не сокращает собственно вычисления модели. Размер результата округляется вниз до чётных пикселей; родной AI-результат ограничен 32768 пикселями по стороне. FPS, звук и CRF применяются как при обычном экспорте. Интерполяции кадров нет.

**Проба до 3 секунд.** Создаёт сравнение от текущей позиции внутри выбранного отрезка; на его конце берёт последние доступные до трёх секунд. Окно «До / После» синхронно воспроизводит два клипа без звука и позволяет перематывать их. Используются выбранные crop, FPS, масштаб и CRF. Проба не заменяет результат полного экспорта. Закрытие окна отменяет обработку пробы и освобождает её файлы.
...
node --test --test-concurrency=1 tests/geometry.test.mjs tests/trim.test.mjs tests/backend.test.mjs tests/ai.test.mjs
...
Проверки `AiChecks` используют тестовый исполнитель и локальный HTTP-сервер: они проверяют установку, обновление, откат, повреждение файлов, отмену на каждом этапе, нехватку места и обработку полной последовательности одним запуском, но не качество моделей. Реальные GPU-проверки запускаются отдельно (скачивают AI-пакеты по необходимости):
...
AI-проба использует существующие `/api/exports/{id}` для статуса, SSE, отмены и удаления. Снимок задачи дополнен `stage`, `framesDone`, `framesTotal`, `preview`. Идентификаторы моделей: `nomos-weak`, `nomos-medium`, `nomos-strong`, `realesrgan`, `anime-video`. Неизвестные модели/масштабы отклоняются с HTTP 400; отсутствующий пакет или занятая обработка — HTTP 409. Пути инструментов и моделей клиент не передаёт.
