...
| `POST /api/exports` | Запуск: `mediaId`, `crop: {x,y,width,height}`, `scale`, `fps`, `audio`, `sourceWidth`, `sourceHeight`, необязательное поле `quality` (`maximum` / `high` / `balanced` / `compact`; отсутствующее или `null` — `maximum`, неизвестное — HTTP 400), необязательные `startSeconds` и `endSeconds` (секунды от начала исходника; по умолчанию — полный файл) |
